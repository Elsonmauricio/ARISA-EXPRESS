        ...
        <div className="grid md:grid-cols-2 gap-6">
              ...
              {routes.map((route) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 rounded-xl border border-gray-300 hover:border-gold/50 cursor-pointer transition-all hover:bg-white"
                  >
          ...
          <div className="glass-strong border-gradient p-6 rounded-2xl flex flex-col justify-center items-center">
            <div className="w-full max-w-sm">
              ...
              <div className="mb-4">
                <label className="block text-sm text-gray-600 mb-1">{t('ship.peso')}</label>
                <input
                  type="number"
                  min="0.1"
                  step="0.1"
                  ...
                  className="w-full px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800"
                />
              </div>
              {selectedRoute ? (
                ...
                <div className="text-center text-gray-400 py-8">{t('ship.selecionarRota')}</div>
              )}
              <GoldButton
      ...
      {step === 'form' && selectedRoute && (
        <div className="max-w-2xl mx-auto">
          <div className="glass-strong border-gradient p-8 rounded-2xl">
            ...
            <div className="bg-[#E8D9F5] p-4 rounded-xl mb-6 text-sm">
              <div className="flex justify-between">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">{t('ship.remetente')}</label>
                  <input type="text" name="senderName" value={formData.senderName} onChange={handleInputChange} required className="w-full px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800" />
                </div>
                 <div>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm text-gray-600 mb-1">{t('ship.comprimento')}</label>
                  <input type="number" name="length" value={formData.length} onChange={handleInputChange} className="w-full px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800" />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">{t('ship.largura')}</label>
                  <input type="number" name="width" value={formData.width} onChange={handleInputChange} className="w-full px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800" />
                </div>
                <div>
                  <label className="block text-sm text-gray-600 mb-1">{t('ship.altura')}</label>
                  <input type="number" name="height" value={formData.height} onChange={handleInputChange} className="w-full px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800" />
                </div>
              </div>

              <div>
                <label className="block text-sm text-gray-600 mb-1">{t('ship.descricao')}</label>
                <textarea name="description" value={formData.description} onChange={handleInputChange} rows={3} className="w-full px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800 resize-none"                   placeholder={t('ship.descricaoPlaceholder')} />
              </div>
...
function ShipmentList() {
  ...
  if (loading) return <div className="text-center py-8 text-gray-600">{t('ship.carregarEncomendas')}</div>;
    ...
    <div className="space-y-8">
      <div className="glass-strong border-gradient p-6 rounded-2xl overflow-x-auto">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-300">
              <th className="text-left py-2 text-gray-600">{t('ship.item')}</th>
              <th className="text-right py-2 text-gray-600">{t('ship.euro')}</th>
              <th className="text-right py-2 text-gray-600">{t('ship.kwanza')}</th>
            </tr>
          </thead>
          <tbody>
            {baseItems.map((row, idx) => (
              <tr key={idx} className="border-b border-lilac/10 hover:bg-white">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-300">
              <th className="text-left py-2 text-gray-600">{t('ship.pesoMalas')}</th>
              <th className="text-right py-2 text-gray-600">{t('ship.euro')}</th>
              <th className="text-right py-2 text-gray-600">{t('ship.kwanza')}</th>
            </tr>
          </thead>
        ...
        <div className="mt-8 pt-4 border-t border-gray-300 text-center">
          <p className="text-sm text-gray-600">{t('ship.maisInfo')}</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 bg-[#E8D9F5] border border-gray-300 rounded-lg focus:border-gold outline-none text-gray-800"
            />
         ...
         {result && (
           <div className="mt-6 space-y-4">
             <div className="flex justify-between items-center border-b border-gray-300 pb-3">
               <div>
                  <div className="text-xs text-gray-400">{t('ship.codigo')}</div>
                 <div className="font-mono text-gold">{result.trackingCode}</div>
               </div>
               <div className="text-right">
                  <div className="text-xs text-gray-400">{t('ship.status')}</div>
             ...
             {result.cttLink && (
               <div>
                 <a
                   ...
                   className="inline-flex items-center gap-2 px-4 py-2 bg-gold text-[#374151] rounded-lg font-semibold text-sm hover:opacity-90 transition-opacity"
                 >
             ...
             <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">{t('ship.origem')}</span>
                <span className="text-gold">{result.origin}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">{t('ship.destino')}</span>
                <span className="text-gold">{result.destination}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">{t('ship.peso')}</span>
                <span className="text-gold">{result.weight} kg</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">{t('ship.precoLabel')}</span>
                <span className="text-gold">{result.price?.toFixed(2) || '->'}</span>
              </div>
              {result.trackingUpdates && result.trackingUpdates.length > 0 && (
                <div className="mt-4">
                  <div className="text-xs text-gray-400 mb-2">{t('ship.historico')}</div>
       ...
       <div className="min-h-screen bg-[#E8D9F5] pt-28 pb-20 px-4">
        <div className="container mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <p className="text-gray-600 mt-2">{t('ship.subtitle')}</p>
          </motion.div>
          ...
          <div className="mt-6">
            {activeTab === 'reservar' && (
              loadingRoutes ? (
                <div className="text-center py-8 text-gray-600">{t('ship.carregarRotas')}</div>
              ) : routesError ? (
            ...
            {activeTab === 'tabela' && (
              loadingRoutes ? (
                <div className="text-center py-8 text-gray-600">{t('ship.carregarRotas2')}</div>
              ) : routesError ? (
                <div className="text-center py-8 text-red-400">{routesError}</div>
              ) : (
                <PriceTable/>
